...
# Reusing connections eliminates TCP/TLS handshake latency (makes requests 2x-3x faster)
http_client = httpx.AsyncClient(
    timeout=30.0, # Reduced to failover faster if a model is stuck
...
def action_clear_memory():
    global history
    history = [{"role": "system", "content": SYSTEM_PROMPT}]
    print("\n🧹 Memory buffer cleared. Ready for new context.\n")

def action_view_stats():
    print("\n🤖 *YoFsociety System Status*")
...
def action_select_model():
    ...
    print("\n⚙️  Architect Dashboard: Select AI Engine")
...
async def main():
    ...
    \ \ / / / _ \|  ___|             (_)    | |        
     \ V / | | | | |__ ___  ___   ___ _  ___| |_ _   _ 
      ...
      \_/   \___/\____/___/\___/ \___|_|\___|\__|\__, |
                                                 ...
                                                 |___/ 
    =========================================================
          Architect: Yoseph | AI Terminal Initialized
    ...
    # Prompt user for API key securely (input characters remain hidden)
    api_key = getpass.getpass("🔑 Please enter your OpenRouter API Key (sk-or-...): ").strip()
    
    if not api_key:
        print("❌ Error: API Key is required to run YoFsociety AI. Exiting...")
        sys.exit(1)
